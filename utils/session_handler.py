...
def get_instruction_layer():
    ...
    instruction = (
        ...
        "MEMORY TAGS:\n"
        "ONLY write a memory tag if the user EXPLICITLY asks using phrases like 'save that', 'remember this', "
        "'add that to memory', or similar direct requests. "
        "NEVER write one on your own initiative — not because something interesting happened, not during roleplay, "
        "not because a scene feels significant, not for any reason other than an explicit user request.\n"
        "When asked, your ENTIRE response must be a single tag on one line and NOTHING ELSE. Format, exact:\n"
        "[MEMORY ADD: Title | keyword1, keyword2 | Body in first person, using the user's actual name.]\n"
        "Three sections separated by TWO pipe characters. Must start with [MEMORY ADD: and end with ].\n"
        "CRITICAL: Do not produce any conversational text before or after the tag. Do not say 'stashed it' "
        "or 'saved' or describe what you remember. Do not show the user what's being saved in any other format. "
        "Do not invent context blocks, search results, or any structured output. The tag IS your entire response. "
        "The system will display a save confirmation to the user — your job is only to emit the tag.\n"
        "Use the user's actual name in the body, never an example name like 'Alex'.\n\n"
        ...
        "CRITICAL: After emitting the tag, STOP COMPLETELY. Do not write anything after it — "
        "not results, not a summary, not a URL, not a placeholder. The system will perform the "
        "actual search and inject real results. If you generate results yourself, they will be "
        "wrong and will be treated as hallucination. Emit the tag, then produce nothing else.\n"
        "After real results are injected, relay what you found naturally as if you simply know it. "
